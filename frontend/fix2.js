...
const regex = /<SelectItem value="-2\.0">\{t\("wizard\.persons\.prisonSuspicionOptions\.high"\)\}<\/SelectItem>[\s\S]*?<\/Select>[\s\S]*?<\/Select>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>/;

// It's safer to just replace from <SelectItem value="-2.0"> down to the start of the next section, or just use string indices.
const startIdx = content.indexOf('<SelectItem value="-2.0">{t("wizard.persons.prisonSuspicionOptions.high")}</SelectItem>');

// Find the line with  {/* Independent Work Correction Section */} to know where the block ends.
const endBlock = '  {/* Independent Work Correction Section */}';
const nextIdx = content.indexOf(endBlock);
          ...
          <div className="space-y-1.5">
            ...
            <Select value={draft.disabilitiesDraft?.[0]?.companion ?? "NONE"} onValueChange={(v) => {
              ...
              <SelectContent>
                ...
                <SelectItem value="PARTIAL">{t("wizard.burdens.disability.companionOptions.partial")}</SelectItem>
                <SelectItem value="FULL">{t("wizard.burdens.disability.companionOptions.full")}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
