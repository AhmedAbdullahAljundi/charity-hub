...
const NATIONAL_ID_RE = /^([23])(\d{2})(0[1-9]|1[012])(0[1-9]|[12]\d|3[01])\d{7}$/;
...
export function WizardShell({
  ...
  const validateBasicInfo = useCallback(() => {
    ...
    const wifeNationalId = fd.wifeNationalId?.trim() ?? "";
    if (!wifeNationalId) {
      toast.error(`${t("wizard.basic.wifeNid")}: ${t("common.required")}`);
      document.getElementById("wifeNationalId")?.focus();
      return false;
    }
    if (!NATIONAL_ID_RE.test(wifeNationalId)) {
      toast.error(t("wizard.basic.nidInvalid"));
      document.getElementById("wifeNationalId")?.focus();
      return false;
    }
  ...
  }, [autoSave, activeTab, setActiveTab, mode, validateBasicInfo]);
  ...
  const handleTabChange = useCallback(async (newTab: 1 | 2 | 3 | 4 | 5) => {
    ...
    setActiveTab(newTab);
  }, [autoSave, activeTab, isDirty, mode, setActiveTab, validateBasicInfo]);
