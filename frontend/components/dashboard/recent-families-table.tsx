...
export interface RecentFamilyRow {
  id: string;
...
interface RecentFamiliesTableProps {
  rows: RecentFamilyRow[];
  loading?: boolean;
}

export function RecentFamiliesTable({ rows, loading }: RecentFamiliesTableProps) {
  ...
  return (
    <Card className="rounded-lg border border-border/50 shadow-sm">
      <CardHeader className="border-b border-border/35 py-2.5 px-3">
        <CardTitle className="text-[11px] font-bold text-foreground">{t("tables.recent_families_title")}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-0 p-0">
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <p className="px-3 py-6 text-center text-[11px] text-muted-foreground">{t("tables.empty_recent")}</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow className="h-8 hover:bg-transparent">
                  ...
                  <TableHead className="text-[10px] font-semibold uppercase text-muted-foreground">{t("tables.vi")}</TableHead>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.id} className="h-9 border-border/30 hover:bg-muted/30">
                    ...
                    <TableCell className="py-1.5 text-xs text-muted-foreground">{row.region || "—"}</TableCell>
                    ...
                    <TableCell dir="ltr" className="py-1.5 text-xs tabular-nums">
                      {row.vulnerabilityIndex != null ? row.vulnerabilityIndex.toFixed(2) : "—"}
                    </TableCell>
                    <TableCell className="py-1.5">
