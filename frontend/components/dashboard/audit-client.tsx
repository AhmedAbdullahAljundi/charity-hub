...
import { ChevronDown, ChevronUp, Download, Inbox } from "lucide-react";
...
export function AuditClient() {
    ...
    <div className="space-y-6" >
      ...
      <div className="flex flex-wrap gap-4 items-end bg-card p-4 rounded-xl border">
        <div className="space-y-1.5 w-36">
          <span className="text-xs font-medium">{t("filterDateRange")} (من)</span>
          <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} />
        </div>
        <div className="space-y-1.5 w-36">
          <span className="text-xs font-medium">{t("filterDateRange")} (إلى)</span>
          <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} />
        </div>
        <div className="space-y-1.5 w-40">
          <span className="text-xs font-medium">{t("filterAction")}</span>
          <Select value={actionFilter} onValueChange={setActionFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="VERIFY_INCOME">{t("actions.VERIFY")}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="space-y-1.5 w-40">
          <span className="text-xs font-medium">{t("filterEntity")}</span>
          <Select value={entityFilter} onValueChange={setEntityFilter}>
            <SelectTrigger><SelectValue placeholder={t("filterAll")} /></SelectTrigger>
            <SelectContent>
              <SelectItem value="ALL">{t("filterAll")}</SelectItem>
              <SelectItem value="Household">{t("entities.Household")}</SelectItem>
              <SelectItem value="IncomeSource">{t("entities.Income")}</SelectItem>
              <SelectItem value="RuleOverride">{t("entities.RuleOverride")}</SelectItem>
              <SelectItem value="User">{t("entities.User")}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="space-y-1.5 w-40">
          <span className="text-xs font-medium">{t("colHousehold")}</span>
          <Input placeholder="..." value={householdCode} onChange={(e) => setHouseholdCode(e.target.value)} />
        </div>
        <div className={locale === "ar" ? "mr-auto" : "ml-auto"}>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="ltr:text-left rtl:text-right">{t("colTime")}</TableHead>
              <TableHead className="ltr:text-left rtl:text-right">{t("colUser")}</TableHead>
              <TableHead className="ltr:text-left rtl:text-right">{t("colAction")}</TableHead>
              <TableHead className="ltr:text-left rtl:text-right">{t("colEntity")}</TableHead>
              <TableHead className="ltr:text-left rtl:text-right">{t("colHousehold")}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
                ...
                return (
                  <React.Fragment key={log.id}>
                    <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => toggleRow(log.id)}>
                      ...
                      <TableCell className="text-xs">{new Date(log.createdAt).toLocaleString(locale === "ar" ? "ar-EG" : "en-US")}</TableCell>
                      <TableCell>{log.user?.name ?? log.userId}</TableCell>
                      <TableCell><Badge variant="outline">{t(`actions.${log.action}` as any) || log.action}</Badge></TableCell>
                      <TableCell className="font-mono text-sm">{t(`entities.${log.entity}` as any) || log.entity}</TableCell>
 ...
 <CardContent className="p-4">
                                              <div className="font-medium mb-2 font-mono text-sm">{t("colField")}: {change.fieldName}</div>
                                              <div className="grid grid-cols-2 gap-2">
                                                <div className="bg-rose-50 border border-rose-100 rounded-md p-2">
                                                  <div className="text-xs text-rose-500 font-bold mb-1">{t("detailsBefore")}</div>
                                                ...
                                                <div className="bg-emerald-50 border border-emerald-100 rounded-md p-2">
                                                  <div className="text-xs text-emerald-500 font-bold mb-1">{t("detailsAfter")}</div>
