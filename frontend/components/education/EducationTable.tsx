...
import { Eye, Pencil, Trash2, ArrowUpDown, ArrowUp, ArrowDown, GraduationCap, Link } from "lucide-react";
...
export default function EducationTable({ list, loading, pagination, searchQuery }: EducationTableProps) {
  ...
  const thClass = "text-start font-semibold p-3 align-middle text-sm border-b border-slate-700 cursor-pointer hover:bg-slate-700/50 transition-colors select-none";
  const tdClass = "p-3 align-middle border-b border-slate-100 text-sm";
        ...
        <table className="w-full text-sm min-w-[1000px] border-collapse relative">
          <thead className="bg-slate-800 text-white sticky top-0 z-10 shadow-sm">
            <tr>
              ...
              <th className={cn(thClass, "text-center cursor-default hover:bg-transparent w-[100px]")}>أكشن</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  <td className={tdClass}><Skeleton className="h-5 w-12" /></td>
                  ...
                  <td className={tdClass}><Skeleton className="h-8 w-24" /></td>
                  <td className={tdClass}><Skeleton className="h-8 w-32" /></td>
                  <td className={tdClass}><Skeleton className="h-8 w-32" /></td>
                  <td className={tdClass}><Skeleton className="h-8 w-12 rounded-full" /></td>
              ...
              list.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                  <td className={tdClass}>
                    <span className="font-mono font-bold text-slate-700" title={r.household.code}>
                  ...
                  <td className={tdClass}>
                    <div className="flex items-start gap-2">
                      <div className={cn("shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5", 
                        r.person.gender === "FEMALE" ? "bg-pink-100 text-pink-700" : "bg-blue-100 text-blue-700"
                      )}>
                        {r.person.age || "?"}
                      </div>
                      <div>
                        <p className="font-bold text-slate-900 leading-tight">{r.person.name}</p>
                        <p className="text-xs text-slate-500 truncate mt-0.5" title={r.household.headName || ""}>
                          عائلة {r.household.headName}
                        </p>
                        <div className="flex gap-1 mt-1">
                          {r.isRepeating && <Badge variant="outline" className="text-[9px] h-4 px-1 border-rose-200 text-rose-700 bg-rose-50">راسب</Badge>}
                          {r.needsLevelUpdate && <Badge variant="outline" className="text-[9px] h-4 px-1 border-amber-200 text-amber-700 bg-amber-50">تحديث</Badge>}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className={tdClass}>
                    <div className="flex flex-col gap-1">
                      ...
                      {r.gradeYear && <span className="text-[11px] text-slate-500 font-medium">الصف {r.gradeYear}</span>}
                      {r.isSpecialEducation && <Badge variant="outline" className="text-[9px] h-4 px-1 border-orange-200 text-orange-700 bg-orange-50 w-fit">تعليم خاص</Badge>}
                    </div>
                  </td>
                  <td className={tdClass}>
                    {r.averageScore !== null ? (
                      <div>
                        <div className="flex items-center justify-between text-xs mb-1">
                          ...
                          <span className="font-mono font-medium">{r.averageScore}%</span>
                        </div>
                  ...
                  <td className={tdClass}>
                    {r.quranJuzCount && r.quranJuzCount > 0 ? (
                      <div>
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="font-bold text-amber-600 bg-amber-50 px-1.5 rounded-sm text-[10px]">
                            {r.quranGrade ? `${r.quranGrade}%` : "-"}
                          </span>
                          <span className="text-xs text-slate-700 font-medium" title={r.quranLastSurah || ""}>
                        ...
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden mb-0.5">
                          <div className="h-full bg-amber-400" style={{ width: `${Math.min(r.quranProgress || 0, 100)}%` }} />
                        </div>
                      </div>
                    ) : <span className="text-slate-400">—</span>}
                  </td>
                  <td className={tdClass}>
                    {r.totalScore !== null ? (
                      <div className="flex items-center gap-2">
                        <div className={cn("w-9 h-9 rounded-full flex flex-col items-center justify-center shadow-sm", getScoreColor(r.totalScore))}>
                        ...
                        <div className="flex flex-col">
                           <span className="text-[10px] text-slate-500">عام</span>
                        </div>
                      </div>
                    ) : <span className="text-slate-400">—</span>}
                  </td>
                  <td className={tdClass}>
                    <span className="text-xs text-slate-600" title={format(new Date(r.updatedAt), "PPP", { locale: arEG })}>
                      {format(new Date(r.updatedAt), "dd/MM/yyyy")}
                    </span>
                  </td>
                  <td className={tdClass}>
                    <div className="flex justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-sky-600 hover:text-sky-700 hover:bg-sky-50" onClick={() => router.push(`/dashboard/education/${r.id}`)}>
