    ...
    if (!family) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <h3 className="text-lg font-semibold text-foreground">{t("notFoundTitle")}</h3>
            ...
            <div className="flex items-center gap-3">
                <Button variant="ghost" size="icon" asChild>
                    <Link href="/dashboard/families">
                        ...
                        <span className="sr-only">{t("backSr")}</span>
                    </Link>
                </Button>
                <div>
                    <h2 className="text-2xl font-bold text-foreground">{family.headName}</h2>
                    <p className="text-muted-foreground text-sm">{family.address} - {getDomainLabel("social_status", family.category)}</p>
                </div>
            </div>
